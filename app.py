...
def get_loyalty_service_id(conn):
    """Returns the service ID for the loyalty reward (free wash).

    If the loyalty service does not exist in the services table, it is created.
    """
    row = conn.execute(
        'SELECT id FROM services WHERE name = ? AND price = ?',
        ('Loyalty Reward', 0.0)
    ).fetchone()
    if row:
        return row['id']
...
def checkin():
    ...
    if request.method == 'POST':
        ...
        loyalty_service_id_form = request.form.get('loyalty_service_id')

        # validate inputs with specific messages
        if not customer_id:
            flash('Customer is not registered – please select or register one first.', 'warning')
            conn.close()
        ...
        customer = conn.execute('SELECT * FROM customers WHERE id = ?', (customer_id,)).fetchone()
        if not customer:
            flash('Selected customer is not registered in the system.', 'danger')
            conn.close()
            return redirect(url_for('checkin'))
        ...
        if is_loyalty_reward:
            if customer['loyalty_points'] < LOYALTY_THRESHOLD:
                flash('Customer does not yet have enough loyalty points for a free wash.', 'warning')
                conn.close()
                return redirect(url_for('checkin'))
